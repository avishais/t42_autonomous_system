...
Y_train_load = Y_train[:,2:]
Y_test_pos = Y_test[:,:2]
Y_test_load = Y_test[:,2:]

num_output_pos = Y_train_pos.shape[1]
num_output_load = Y_train_load.shape[1]
...
Y_pos = tf.placeholder("float", [None, num_output_pos])
Y_load = tf.placeholder("float", [None, num_output_load])

# Store layers weight & bias
weights_pos, biases_pos = wNb(state_action_dim, hidden_layers, num_output_pos)
weights_load, biases_load = wNb(state_action_dim, hidden_layers, num_output_load)

# Construct model
keep_prob_input = tf.placeholder(tf.float32)
keep_prob = tf.placeholder(tf.float32)
if not DropOut:
    prediction_pos = neural_net(X, weights_pos, biases_pos, activation)
    prediction_load = neural_net(X, weights_load, biases_load, activation)
else:
    # X_drop = tf.nn.dropout(X, keep_prob=keep_prob_input)
    prediction_pos = neural_net_dropout(X, weights_pos, biases_pos, keep_prob, activation)
    prediction_load = neural_net_dropout(X, weights_load, biases_load, keep_prob, activation)

# Define loss 
cost_pos = tf.reduce_mean(0.5*tf.pow(prediction_pos - Y_pos, 2))#/(2*n)
cost_load = tf.reduce_mean(0.5*tf.pow(prediction_pos - Y_load, 2))#/(2*n)
cost = cost_pos + cost_load
...
with open('../models/' + simORreal + '_param.obj', 'wb') as f: 
    pickle.dump([x_mean, x_std, DropOut, Regularization, hidden_layers, activation, state_action_dim, state_dim, num_output_pos, num_output_load, model_file], f)
...
with tf.Session() as sess:
    ...
    if training:
        ...
        for i in range(1, num_steps+1):
            ...
            batch_y_pos = batch_y[:,:2]
            batch_y_load = batch_y[:,2:]

            # Run optimization op (backprop) and cost op (to get loss value)
            if not DropOut:
                _, c = sess.run([train_op, cost], feed_dict={X: batch_x, Y_pos: batch_y_pos, Y_load: batch_y_load})
            else:
                _, c = sess.run([train_op, cost], feed_dict={X: batch_x, Y_pos: batch_y_pos, Y_load: batch_y_load, keep_prob_input: 0.5, keep_prob: 0.5})
    ...
    training_cost = sess.run(cost, feed_dict={X: X_train, Y_pos: Y_train_pos, Y_load: Y_train_load, keep_prob_input: 1.0, keep_prob: 1.0})
    ...
    Y_test_pos_pred, Y_test_load_pred = sess.run([prediction_pos, prediction_load], {X: X_test, keep_prob_input: 1.0, keep_prob: 1.0})
    Y_test_pred = np.concatenate((Y_test_pos_pred, Y_test_load_pred), axis=1)
