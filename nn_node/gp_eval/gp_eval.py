...
with open('t42_cyl35_data_discrete_v0_d4_m1_episodes.obj', 'rb') as f: 
    D = pickle.load(f)
...
if 0:
    ...
    for k in range(N):
        ...
        i = 0
        for a in A:
            state = o_srv(state.reshape(-1,1), a).next_state
    ...
    with open('error_points.pkl', 'wb') as f: 
        pickle.dump([O, E, l], f)
else:
    with open('error_points.pkl', 'rb') as f: 
        O, E, l = pickle.load(f)
...
for e, o in zip(Etest, Otest):
    print i
    idx = kdt.query(o.reshape(1,-1), k = K, return_distance=False)
    O_nn = Otrain[idx,:].reshape(K, 10)
